...
module.exports = function(app, device){
  ...
  app.get('/devices/:userNo', function(req, res){
    device.find({user:{ $elemMatch:{ "userNo" : req.params.userNo} } },
    { "user" :{ $elemMatch:{ "userNo" : req.params.userNo} },"user.password" : false },
    function (err, result){
      if(err){
        return res.status(500).send({ error : "Database Connection Fail" });
      }
  ...
  app.get('/login/:userNo/:password', function(req, res){
    device.find({ user: { $elemMatch :
      {"userNo" : req.params.userNo, "password" : req.params.password} } },
      { "userNo" : true, "username" : true },
      function(err, result){
        if(err){
          return res.status(500).send({ error : "Database Connection Fail" });
        }
    ...
    app.get('/devices/check/:deviceNo', function(req, res){
      device.find({"deviceNo" : req.params.deviceNo}, function(err, result){
        if(err){
          return res.status(500).send({ error : "Database Connection Fail" });
        }
        if(result.length === 0){
    ...
    app.put('/rent', function(req, res){
      ...
      device.findOne({user: { $elemMatch:{ "userNo" : req.body.userNo } } }, function(err, next){
          if(err) return res.status(500).json({ error: "Database Connection Fail" });
          ...
          if(req.body.userNo === "nt11253"){
            device.update({
              ...
              {$set:{
                ...
                if(!output.n) return res.status(404).json({ error: "userNo not found / deviceNo     not found" });
